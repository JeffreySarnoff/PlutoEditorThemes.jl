...
Resource(src::AbstractString, html_attributes::Pair...) =
    Resource(src, mime_fromfilename(src), html_attributes)
...
function mime_fromfilename(
    filename;
    default::T = nothing,
    filename_maxlength = 2000,
)::Union{MIME,T} where {T}
    if length(filename) > filename_maxlength
        default
    else
        MIMEs.mime_from_path(URIs.URI(filename).path, default)
    end
end
